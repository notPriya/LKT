...
% Save off the orientations.
TrackedObject.theta = zeros(n, 3);
% Time results.
...
for i = start:start+n-1
    ...
    % Update the estimate of M.
    M_update = warp.newWarp([0; 0; 0; roll(i)-roll(max(1, i-1))]);
    M = warp.compose(M, M_update);
    ...
    lkt_theta = M(1, 2)/alpha;
    imu_theta = pose(i, 1);
    
    beta = 0.6;
    theta = beta * imu_theta + (1-beta) * lkt_theta;
    ...
    % Orientation results.
    TrackedObject.theta(index, :) = [lkt_theta imu_theta theta];
    % Time results.
...
save([pipe_name '_imu_results.mat'], 'pos');
